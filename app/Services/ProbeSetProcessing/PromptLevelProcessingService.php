...
class PromptLevelProcessingService implements ProbeSetProcessingServiceInterface
{
    ...
    protected function performWithoutFrame($data, $rule, $key, $result, $consecutiveCriteria)
    {
        ...
        return [
            'success' => true,
            'message' => 'Prompt Level probe processed successfully.',
        ];
    }
    ...
    protected function performProcessing($data, $processedData, $nextPhaseId, $isProgramChanged, $isRetained, $isDOIRequired)
    {
        ...
        // DOI is required need to calculate doi for Prompt level      

        if ($isDOIRequired) {
            $collectedData = json_decode($data['collected_data'], true);
            $inputs = $collectedData['inputs']['choices'];

            // Construct the prompt levels array dynamically
            $promptLevels = array_map(function ($choice) {
                return $choice['value'];
            }, $inputs);
            $calculateDOI = $this->collectionModel->calculateDOIForPromptLevels(
                $data['client_id'],
                $data['target_id'],
                $data['client_probe_set_id'],
                $data['current_phase_id'],
