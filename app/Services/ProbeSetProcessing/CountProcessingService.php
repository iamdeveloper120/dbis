...
class CountProcessingService implements ProbeSetProcessingServiceInterface
{
    ...
    protected function performProcessing($data, $processedData, $nextPhaseId, $isProgramChanged, $isRetained, $isDOIRequired)
    {
        ...
        if ($isDOIRequired) {
            ...
            $minValue = $collectedData['inputs']['range']['min'];
            $maxValue = $collectedData['inputs']['range']['max'];

            $calculateDOI =  $this->collectionModel->calculateDOIForCountProbes(
                $data['client_id'],
                $data['target_id'],
                $data['client_probe_set_id'],
                $data['current_phase_id'],
