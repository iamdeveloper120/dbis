...
class DurationProcessingService implements ProbeSetProcessingServiceInterface
{
    ...
    protected function performProcessing($data, $processedData, $nextPhaseId, $isProgramChanged, $isRetained, $isDOIRequired)
    {
        ...
        if ($isDOIRequired) {
            ...
            $durationSet = array_map(function ($choice) {
                return $choice['value'];
            }, $inputs);
            $calculateDOI = $this->collectionModel->calculateDOIForDuration(
                $data['client_id'],
                $data['target_id'],
                $data['client_probe_set_id'],
                $data['current_phase_id'],
