...
class TrafficLightProcessingService implements ProbeSetProcessingServiceInterface
{
    ...
    public function process(array $data, array $collectedData): array
    {
        ...
        log_message('info', $key . '-Consecutive-' . $consecutiveCriteria);
    ...
    protected function performWithoutFrame($data, $rule, $key, $result, $consecutiveCriteria)
    {
        ...
        if ($rule['same_day_check']) {
            ...
            log_message('info', $key . '-consecutiveDaysMet-' . $consecutiveDaysMet);
            if ($consecutiveDaysMet < $consecutiveCriteria) {
                ...
                log_message('info', $key . '-$consecutiveDaysMet < $consecutiveCriteria-');
            } else {
                // Process according to the same_day_check rule
                ...
                $isProgramChanged = false;
                log_message('info', $key . '-isCriteriaMet-' . ($isCriteriaMet ? 'true' : 'false'));
        ...
        return [
            'success' => true,
            'message' => 'Traffic Light probe processed successfully.',
        ];
    }
