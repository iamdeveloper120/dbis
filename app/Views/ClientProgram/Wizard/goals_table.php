 <div class="table-responsive">
     <table id="goal_dataTable" class="table table-bordered align-middle" style="width:100%">
         <thead>
             <tr>
                 <th>Domain Code</th>
                 <th>Domain Name</th>
         ...
         <tbody>
             <?php foreach ($goals as $goal) : ?>
                 <tr>
                     <td><?= $goal->domain_code; ?></td>
                     <td><?= $goal->domain_name; ?></td>
                     <td><?= $goal->goal_code; ?></td>
                     <td><?= $goal->name; ?></td>
                     <td class="dt-nowrap">
                         <button id="<?= $goal->id; ?>" type="button" class="btn btn-sm btn-outline-primary addToClient"><i class="ri-user-add-line align-bottom me-1"></i>Add To Client</button>
                     </td>
                 </tr>
             <?php endforeach; ?>
         </tbody>
 ...
 <script>
     $(document).ready(function() {
         table = $('#goal_dataTable').DataTable({
             lengthChange: false,
             lengthMenu: [
                 [10, 25, 50, -1],
         ...
         /***************************************************************************************** */
         $("#goal_dataTable").on('click', '.addToClient', function(e) {
             var btn = $(this);
             var id = $(this).attr('id');
             ...
             Swal.fire({
                 ...
                 if (result.isConfirmed) {
                     var ajaxRequest = $.ajax({
                         url: '<?php echo base_url() ?>client-program/wizard/goals/create',
                         type: 'post',
                         data: {
                             "id": id,
                             "client_id": client_id
