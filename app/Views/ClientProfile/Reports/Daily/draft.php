...
<?php
    ...
    $dailyListUrl = $daily_list_url ?? base_url('client-profile');
    $profileVersionBaseUrl = base_url('client-profile/reports/daily/' . esc($encodedClientId) . '/version');
?>
        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="progress-paper">
                    ...
                    <div class="daily-comment-section">
                        ...
                        <div class="input-group mb-2">
                            <input type="file" class="form-control" id="daily_image_upload" accept=".jpg,.jpeg,.png,image/jpeg,image/png" multiple <?= auth()->user()->can('client-profile.reports.daily.save-draft') ? '' : 'disabled' ?>>
                            <?php if (auth()->user()->can('client-profile.reports.daily.save-draft')): ?>
                ...
                <div class="mt-4 d-flex gap-2 flex-wrap">
                    ...
                    <?php endif; ?>
                    <?php if (auth()->user()->can('client-profile.reports.daily.finalize')): ?>
...
<script>
    $(document).ready(function() {
        ...
        var loaderCounter = 0;
        var canSaveDraft = <?= auth()->user()->can('client-profile.reports.daily.save-draft') ? 'true' : 'false' ?>;
        ...
        var versionBaseUrl = '<?= esc($profileVersionBaseUrl, 'js') ?>';
        ...
        function validateImageFiles(files, replaceMode) {
            ...
            if (!replaceMode && (dailyImagesStore.length + files.length) > dailyImageMaxCount) {
                return { ok: false, message: 'Maximum allowed images is ' + dailyImageMaxCount + '.' };
            }
        ...
        function fetchDailyImages(useLoader) {
            ...
            var request = useLoader
                ? postWithLoader(versionBaseUrl + '/' + versionId + '/images', {})
                : $.post(versionBaseUrl + '/' + versionId + '/images', {});
        ...
        function uploadDailyImages(files) {
            ...
            showPageLoader();
            $.ajax({
                url: versionBaseUrl + '/' + versionId + '/images/upload',
                type: 'post',
                data: formData,
                processData: false,
                contentType: false
        ...
        function deleteDailyImage(artifactId) {
            ...
            postWithLoader(versionBaseUrl + '/' + versionId + '/images/' + artifactId + '/delete', {})
                .done(function(response) {
                    if (!response || response.status !== 'success') {
                        showAlert(
                            (response && response.statusText) ? response.statusText : 'Daily Report',
        ...
        function replaceDailyImage(file) {
            ...
            showPageLoader();
            $.ajax({
                url: versionBaseUrl + '/' + versionId + '/images/' + dailyReplaceArtifactId + '/replace',
                type: 'post',
                data: formData,
                processData: false,
                contentType: false
        ...
        $('#btn_fetch_daily_content').on('click', function() {
            ...
            postWithLoader(versionBaseUrl + '/' + versionId + '/pull-section', {
        ...
        $('#btn_save_draft').on('click', function() {
            ...
            postWithLoader(versionBaseUrl + '/' + versionId + '/save-draft', {
        ...
        $('#btn_finalize_draft').on('click', function() {
            ...
            Swal.fire({
                ...
                postWithLoader(versionBaseUrl + '/' + versionId + '/finalize', {})
                    .done(function(response) {
                        if (!response || response.status !== 'success') {
