...
<?php
    ...
    $canManageInstructionalImages = auth()->user()->can('reporting.progress.save-draft');
    ...
    $progressListUrl = base_url('reports/progress') . (string) ($list_state_query ?? '');
?>
...
<script>
    $(document).ready(function() {
        ...
        var canSaveDraft = <?= auth()->user()->can('reporting.progress.save-draft') ? 'true' : 'false' ?>;
        ...
        function postSectionStateUpdate(sectionKey, sectionData, extraPayload) {
            ...
            return postWithLoader(
                '<?= base_url('reports/progress/version') ?>/' + versionId + '/update-section-state',
                requestPayload
            );
        }
        ...
        function fetchInstructionalImages(useLoader) {
            ...
            var request = useLoader
                ? postWithLoader('<?= base_url('reports/progress/version') ?>/' + versionId + '/instructional-images', {})
                : $.post('<?= base_url('reports/progress/version') ?>/' + versionId + '/instructional-images', {});
        ...
        function uploadInstructionalImages(files) {
            ...
            showPageLoader();
            $.ajax({
                url: '<?= base_url('reports/progress/version') ?>/' + versionId + '/instructional-images/upload',
                type: 'post',
                data: formData,
                processData: false,
                contentType: false
        ...
        function deleteInstructionalImage(artifactId) {
            ...
            postWithLoader('<?= base_url('reports/progress/version') ?>/' + versionId + '/instructional-images/' + artifactId + '/delete', {})
                .done(function(response) {
                    if (!response || response.status !== 'success') {
                        showAlert(
                            (response && response.statusText) ? response.statusText : 'Progress Report',
        ...
        function replaceInstructionalImage(file) {
            ...
            showPageLoader();
            $.ajax({
                url: '<?= base_url('reports/progress/version') ?>/' + versionId + '/instructional-images/' + instructionalReplaceArtifactId + '/replace',
                type: 'post',
                data: formData,
                processData: false,
                contentType: false
        ...
        $('.btn-pull-placeholder').on('click', function() {
            ...
            postWithLoader('<?= base_url('reports/progress/version') ?>/' + versionId + '/pull-section', {
        ...
        $('#btn_save_draft').on('click', function() {
            ...
            postWithLoader('<?= base_url('reports/progress/version') ?>/' + versionId + '/save-draft', payload())
                .done(function(response) {
        ...
        $('#btn_finalize_draft').on('click', function() {
            ...
            Swal.fire({
                ...
                $.post('<?= base_url('reports/progress/version') ?>/' + versionId + '/save-draft', payload())
                    .done(function(saveResponse) {
                        ...
                        $.post('<?= base_url('reports/progress/version') ?>/' + versionId + '/finalize', {})
                            .done(function(response) {
                                if (response.status !== 'success') {
