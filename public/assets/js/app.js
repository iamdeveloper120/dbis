  ...
  function initActiveMenu() {
    ...
    if (currentPath) {
      ...
      /*if (document.getElementById("navbar-nav") != null)
        a = document
          .getElementById("navbar-nav")
          .querySelector(`a[href="${currentPath}"], a[href="${currentPath}/"]`);*/

      // Normalize the current path by removing the trailing slash if it exists
      if (currentPath.endsWith("/")) {
        currentPath = currentPath.slice(0, -1);
      }

      // Define custom mappings with patterns for sub-pages
      var customMappings = [
        { main: "/sessions/daily", pattern: /^\/sessions\/review(\/.*)?$/ }, // Pattern for /sessions/review and any sub-pages
        {
          main: "/master-program/domains",
          pattern: /^\/master-program\/(goals|targets)/,
        }, // Only match goals and targets
        {
          main: "/user-configuration/users/active-list",
          pattern: /^\/user-configuration\/users\/(active-list|edit(\/.*)?|new)$/,
        },
        {
          main: "/user-configuration/users/inactive-list",
          pattern: /^\/user-configuration\/users\/inactive-list$/,
        },
      ];

      // Exclude certain paths explicitly
      var excludePatterns = [
        /^\/master-program\/phases-rules-setup$/, // Exclude /master-program/phases-rules-setup
      ];
      // Variable to store the best match
      var bestMatch = null;

      // Check if the current path matches any exclusion patterns
      var isExcluded = excludePatterns.some(function (pattern) {
        return pattern.test(currentPath);
      });

      // If the path is not excluded, proceed with custom mappings
      if (!isExcluded) {
        customMappings.some(function (mapping) {
          if (mapping.pattern && mapping.pattern.test(currentPath)) {            
            bestMatch = document.querySelector(`a[href="${mapping.main}"]`);            
            return true; // Stop processing after the first valid match
          }
          return false;
        });
      }

      // If no match was found in custom mappings, proceed with regular path matching
      if (!bestMatch) {
        var sidebarLinks = document.querySelectorAll("#navbar-nav a");

        // Iterate over all sidebar links
        sidebarLinks.forEach(function (link) {
          var linkHref = link.getAttribute("href");

          // Normalize the link by removing any trailing slash
          ...
          if (currentPath.startsWith(linkHref)) {
            // Select the longest matching link to ensure we pick the most specific match
            if (
              !bestMatch ||
              linkHref.length > bestMatch.getAttribute("href").length
            ) {
              bestMatch = link;
            }
          }
